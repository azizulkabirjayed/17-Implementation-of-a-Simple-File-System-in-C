...
{
    return ((long)INODE_TABLE_BLOCK * BLOCK_SIZE)
         + ((long)(inode_number - 1) * sizeof(inode_t));
}

int data_bitmap_index(int absolute_block_number)
{
    return absolute_block_number - DATA_REGION_BLOCK;
}
...
int filename_exists(FILE *image, const char *filename)
{
    ...
    for (int i = 0;
         i < BLOCK_SIZE / (int)sizeof(dirent_t);
         i++) {
...
int main(int argc, char *argv[])
{
    ...
    if (strlen(source_name) > 58) {
        printf(
            "Error: file name is too long for SimpleFS.\n"
        );
        fclose(source);
        fclose(image);
        return 1;
    ...
    for (int i = 0; i < required_blocks; i++) {
        int block =
            find_free_data_block(data_bitmap);

        if (block == -1) {
            printf(
                "Error: insufficient free data blocks.\n"
            );
            fclose(source);
            fclose(image);
            return 1;
    ...
    directory_entry_index =
        find_free_directory_entry(image);
    ...
    for (int i = 0; i < required_blocks; i++) {
        ...
        if (fseek(
            ...
            printf(
                "Error: could not write file data to image.\n"
            );
    ...
    {
        long pos =
            ((long)ROOT_DATA_BLOCK *
             BLOCK_SIZE)
            +
            ((long)directory_entry_index *
             sizeof(dirent_t));
